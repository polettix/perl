...
modulo the caveats given in L<perlipc/"Signals">.
	...
	alarm $timeout;
	$nread = sysread $sock, $buffer, $size;
	alarm 0;
    };
    if ($@) {
...
Example:
    ...
    open(my $outfh, '|sort >foo')  # pipe to sort
        or die "Can't start sort: $!";
    #...			# print stuff to $outfh
    close $outfh		# wait for sort to finish
        or warn $! ? "Error closing sort pipe: $!"
                   : "Exit status $? from sort";
    open(my $infh, '<', 'foo') # get sort's results
        or die "Can't open 'foo' for input: $!";
    #...       # read stuff from $infh
...
before you call dbmopen():
    ...
    use DB_File;
    dbmopen(my %NS_Hist, "$ENV{HOME}/.netscape/history.db")
...
file.  Manual error checking can be done this way:
    ...
    for my $file ("/share/prog/defaults.rc",
                  "$ENV{HOME}/.someprogrc")
   {
...
Here's an example of setting a filehandle C<$remote> to be
...
same underlying descriptor:
    ...
    if (fileno($thisfh) == fileno($thatfh)) {
	print "\$thisfh and \$thatfh are dups\n";
    }
...
In the 2-arguments (and 1-argument) form,
...
Examples:
    ...
    open(my $fh, '-|', "caesar <$article")     # decrypt article
	or die "Can't start caesar: $!";

    open(my $fh, "caesar <$article |")		# ditto
	or die "Can't start caesar: $!";

    open(my $fh, "|sort >Tmp$$")		# $$ is our process id
	or die "Can't start sort: $!";
...
Here is a script that saves, redirects, and restores C<STDOUT>:
...
If you have to retain compatibility with previous versions of Perl,
you might need to avoid lexical filehandles (available as of Perl
5.6.0) and the three-arguments C<open()> form (available as of Perl
5.8.0), so a temporary redirection of STDERR on STDOUT would be
like this:
...
or, if you're starting from a "legacy" filehandle:
...
C<< open(my $A, '>>&', $B) >>, the filehandle C<$A> will not have the same file
descriptor as C<$B>, and therefore flock($A) will not flock($B), and vice
versa.  But with C<< open(my $A, '>>&=', $B) >> the filehandles will share
...
The following quadruples are more or less equivalent:
...
will have exactly the opposite restrictions. In general,
stick to the 3-arguments form which is safer, and switch to the 2-arguments
form when you really need it.
...
another way to protect your filenames from interpretation.  For example:
    ...
    use IO::Handle; # for autoflush() method on filehandles
    use Fcntl;      # for constants O_RDWR, O_CREAT and O_EXCL
...
The possibility to have scalar variables for filehandles has been added
in Perl 5.6.0. Anyway, you can use an undefined variable to hold the filehandle
only starting from Perl 5.8.0 on; in previous releases, you should
use the constructor from the C<IO::Handle> package (or one of its
subclasses, such as C<IO::File> or C<IO::Socket>). For example:
...
In almost all examples above we used lexical scalar variables for filehandles; this
restricts the scope of the filehandle itself, avoiding potential clashing
between different parts of the program, and has the added benefit to
automatically close whenever and however you leave that scope:
	...
	return $first, <$fh> if $ALL;	# Or here.
	$first;					# Or here.
    }
...
Here is an example to illustrate how the bits actually fall in place:
    ...
    my ($off, $width, $bits, $val, $res);
    for my $w (0..3) {
        $width = 2**$w;
        for (my $shift=0; $shift < $width; ++$shift) {
